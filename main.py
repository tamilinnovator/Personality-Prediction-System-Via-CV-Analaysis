...
import nltk
nltk.download('C:/Users/esakk/AppData/Roaming/nltk_data')
...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
    ...
    Label(result, text = str('{} : {}'.format("Name:", aplcnt_name.get())).title(), foreground='white', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Age:", age)), foreground='white', bg='white', anchor='w').pack(fill=BOTH)
    for key in data.keys():
        if data[key] is not None:
    ...
    Label(result, text = str("perdicted personality: "+personality).title(), foreground='white', bg='white', anchor='w').pack(fill=BOTH)
...
def OpenFile(b4):
    global loc;
    name = filedialog.askopenfilename(initialdir="C:/Users/esakk",
